...
> | P0 — live UI keeps only sensor values; failures silent | **Closed for the live view.** The client keeps value, timestamp and quality; only good, fresh readings colour a zone, a grey zone says why ("no reading · 4 min", "reading flagged"), and live load reports how many meters it rests on ([§55](decisions.md#55-the-live-map-judges-a-reading-before-it-draws-one)). Verified in a browser with injected `offline` and `spike` faults. The rollups no longer average flagged samples either: migration 016 rebuilds all three over `quality = 0`, counter included ([§57](decisions.md#57-a-flagged-sample-is-counted-in-the-rollup-and-kept-out-of-its-statistics)) |
> | P0 — alert events use the same backlog-skipping delivery as telemetry | **Closed.** An alert frame is never skipped: a client too backlogged to take one is closed with 1013 and reconciles over HTTP on reconnect. Sending anyway would queue without bound; skipping was the defect ([§56](decisions.md#56-an-alert-frame-is-never-skipped-a-client-too-slow-for-one-is-disconnected)) |
> | P0 — a resolved alert stays visible if it was in the initial HTTP snapshot | **Closed.** The list is the snapshot reconciled with events — the snapshot is the truth about everything before it was requested, events about everything after — and it is refetched on every accepted subscription and every five minutes. Verified in a browser: an alert open at page load left the list when it resolved, with no reload and no refetch in between |
...
> | Not in the table — no rate limiting anywhere | **Closed.** Per-address failed authentication, per-key requests, per-TENANT readings (the write buffer sheds oldest without asking whose rows they are, so this is the fairness mechanism), per-connection frames, per-tenant sockets, and sign-in failures plus a ceiling on concurrent password hashes ([§54](decisions.md#54-rate-limits-are-per-resource-keyed-by-whoever-can-exhaust-it)). Found on the way: `login()` held a pooled connection for the whole of each hash |
